RSpec.describe Veeqo::HttpErrors do
  let(:dummy_class) { Class.new { extend Veeqo::HttpErrors } }
  let(:code) { 200 }
  let(:env) { double }
  let(:body) { {} }
  let(:headers) { {} }
  ...
  it '::ERRORS is not nil' do
    expect(Veeqo::HttpErrors::ERRORS).not_to be_nil
  end
  ...
  context 'invalid response status' do
    context 'when we get a 404' do
      ...
      it 'should throw an exception' do
        expect { dummy_class.throw_http_exception!(code, env) }.to raise_exception(Veeqo::HttpErrors::ERRORS[code])
      end
    end

    it 'should have a valid error' do
      Veeqo::HttpErrors::ERRORS.keys.each do |code|
        expect { dummy_class.throw_http_exception!(code, env) }.to raise_exception(Veeqo::HttpErrors::ERRORS[code])
      end
    end
    ...
    context 'when have a body and response headers' do
      ...
      it 'should parse out a retry-after header if present' do
        ...
        rescue Veeqo::TooManyRequests => e
